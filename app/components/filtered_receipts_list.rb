#
# Receipt rows list/grid component implementation with date filtering support
#
# - author: Steve A.
# - vers. : 0.35.20130221
#
class FilteredReceiptsList < Netzke::Basepack::BorderLayoutPanel
  ...
  component :list_view_grid do
    {
      ...
      :scope => ( config[:patient_id] ?
          [
            "patient_id = ? AND ( (DATE_FORMAT(date_receipt,'#{AGEX_FILTER_DATE_FORMAT_SQL}') >= ? AND DATE_FORMAT(date_receipt,'#{AGEX_FILTER_DATE_FORMAT_SQL}') <= ?) OR date_receipt IS NULL )",
            config[:patient_id],
          ...
          [
            "DATE_FORMAT(date_receipt,'#{AGEX_FILTER_DATE_FORMAT_SQL}') >= ? AND (DATE_FORMAT(date_receipt,'#{AGEX_FILTER_DATE_FORMAT_SQL}') <= ? OR date_receipt IS NULL)",
            component_session[:filtering_date_start] ||= config[:filtering_date_start],
            component_session[:filtering_date_end] ||= config[:filtering_date_end]
          ]
      )
